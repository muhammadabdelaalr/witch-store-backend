...
async function runMigration() {
    ...
    let sqliteDb;
    try {
        sqliteDb = new better_sqlite3_1.default(dbPath, { readonly: true });
    }
    catch (err) {
    ...
    try {
        ...
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "sale_items" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "sales" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "customer_transactions" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "supplier_transactions" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "expenses" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "products" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "categories" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "customers" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "suppliers" CASCADE;`);
        await prisma_1.prisma.$executeRawUnsafe(`TRUNCATE TABLE "users" CASCADE;`);
        ...
        const saleItems = sqliteDb.prepare('SELECT * FROM sale_items').all();
        for (const si of saleItems) {
            await prisma_1.prisma.saleItem.create({
                data: {
                    id: si.id,
                    sale_id: si.sale_id,
        ...
        const customerTransactions = sqliteDb.prepare('SELECT * FROM customer_transactions').all();
        for (const ct of customerTransactions) {
            await prisma_1.prisma.customerTransaction.create({
                data: {
                    id: ct.id,
        ...
        const supplierTransactions = sqliteDb.prepare('SELECT * FROM supplier_transactions').all();
        for (const st of supplierTransactions) {
            await prisma_1.prisma.supplierTransaction.create({
                data: {
                    id: st.id,
